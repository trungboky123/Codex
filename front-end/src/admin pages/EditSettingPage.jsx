...
import s from "../css/EditSetting.module.scss";
...
export default function EditSettingPage() {
  ...
  async function fetchSetting() {
    const res = await authFetch(`http://localhost:8080/settings/${id}`, {
      method: "GET",
    });
    const data = await res.json();
    setSetting(data);
  ...
  const handleTypeSelect = (typeId) => {
    ...
    setSetting((prev) => ({ ...prev, parent: selectedType }));
  ...
  const selectedStatus = statusOptions.find((s) => s.value === setting.status);
  const isTopLevel = setting.parent === null;
  ...
  async function handleSave(e) {
    ...
    const res = await authFetch(`http://localhost:8080/settings/update/${id}`, {
      method: "PATCH",
            ...
            <div className={s.formGrid}>
              ...
              <div className={s.previewCard}>
                ...
                <div className={s.previewBox}>
                  <div className={`${s.previewOrb} ${s.orbDefault}`}>
                  ...
                  <div className={s.previewMeta}>
                    ...
                    <div className={s.previewTags}>
                      ...
                      <span
                        className={`${s.statusDot} ${setting.status ? s.dotActive : s.dotInactive}`}
                      >
                ...
                <div className={s.infoChips}>
                  ...
                  <div className={s.chip}>
                    ...
                    <span>Type: </span>
                    <strong>{setting.parent?.name || "—"}</strong>
                  </div>
                  <div className={s.chip}>
                    <i
                      className="bi bi-circle-fill"
                      style={{
                        color: setting.status ? "#10b981" : "#ef4444",
                        fontSize: "0.55rem",
                      }}
                    ></i>
                    <span>Status: </span>
                    <strong>{setting.status ? "Active" : "Inactive"}</strong>
                  </div>
                </div>
              </div>
              ...
              <div className={s.fieldsCard}>
                ...
                <div className={s.formGroup}>
                  <label className={s.label}>
                    ...
                    {isTopLevel && (
                      <span className={s.disabledNote}>
                        ...
                        This setting is a top-level type
                      </span>
                    )}
                  </label>
                  <div className={s.dropdown} ref={typeDropdownRef}>
                    ...
                    {!isTopLevel && typeDropdownOpen && (
                      <div className={s.filterDropdownMenu}>
                        {settingTypes.map((type) => (
                          <button
                            key={type.id}
                            type="button"
                            className={`${s.filterDropdownItem} ${setting.parent?.id === type.id ? s.active : ""}`}
                            onClick={() => handleTypeSelect(type.id)}
                          >
                            <span className={s.itemLeft}>
                              <span className={s.itemText}>
                                <span className={s.itemLabel}>{type.name}</span>
                              </span>
                            </span>
                            {setting.parent?.id === type.id && (
